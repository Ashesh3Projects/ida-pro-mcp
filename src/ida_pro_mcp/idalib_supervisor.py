...
def idb_open(
    ...
    mode: Annotated[
        str,
        "How to open: prefer_headless (default; idalib worker), "
        "force_headless (idalib worker only)."
    ] = "prefer_headless",
    ...
    preferred_session_id: Annotated[
        str, "Preferred session ID (auto-generated if empty). Ignored if the file is already open in a worker session."
    ] = "",
) -> IdalibOpenResult:
    """Open a binary and warm it up. Returns the existing session if the file is already open under the supervisor; otherwise creates a worker session."""
...
def idb_list() -> IdalibListResult:
    """List sessions opened through this supervisor."""
    sup = _require_supervisor()
    try:
