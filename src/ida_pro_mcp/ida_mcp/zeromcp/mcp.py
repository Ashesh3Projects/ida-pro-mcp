...
class McpHttpRequestHandler(BaseHTTPRequestHandler):
    ...
    def __init__(self, request, client_address, server):
        ...
        try:
            ...
            # stop() can close the native socket during setup, read, or finish.
            if self.mcp_server._running:
                raise
        finally:
            ...
            while conn.alive and self.mcp_server._running:
                ...
                if sock:
                    try:
                        ...
                        # Windows stop() may detach after the loop condition.
                        if self.mcp_server._running:
                            raise
                        break
                else:
...
class McpServer:
    ...
    def _register_http_connection(self, connection: socket.socket) -> None:
        ...
        try:
            ...
            pass
        # Registration lost the race with stop(); no handler owns rfile yet.
        connection.close()
    ...
    def _shutdown_http_connections(self) -> None:
        ...
        for connection in connections:
            ...
            if sys.platform == "win32":
                # Winsock shutdown does not wake Python's timeout-mode select.
                # close() alone also defers the native close while rfile owns
                # a makefile reference, so detach and close the handle directly.
                try:
